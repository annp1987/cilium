...
type IPCache struct {
	Mutex              lock.RWMutex
	ipToIdentityCache  map[string]NumericIdentity
...
// within ipc.
func (ipc *IPCache) Upsert(endpointIP string, identity NumericIdentity) {
	ipc.Mutex.Lock()
	...
	ipc.xdsResourceMutator.Upsert(envoy.NetworkPolicyHostsTypeURL, identity.StringID(), &api.NetworkPolicyHosts{uint64(identity), ipStrings}, false)
...
// in the IPCache.
func (ipc *IPCache) LookupByIP(endpointIP string) (NumericIdentity, bool) {
	ipc.Mutex.RLock()
